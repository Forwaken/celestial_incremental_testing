...
addNode("tsi", {
    ...
    canClick: true,
    onClick() {
        player.subtabs["tad"]["Domain"] = "Simplification"
    },
    layerShown() {return player.tad.altInfinities.distorted.milestone.gte(3)},
})
addNode("tma", {
    color: "#6d228b",
    symbol: "Ma",
    ...
    layerShown() {return player.tad.altInfinities.corrupted.milestone.gte(3)},
    hotkeys: [
        {
            key: "q", 
            description: "Magnify",
...
addLayer("tad", {
    ...
    universe: "U2",
    innerNodes: [["tac", "tam"], ["tco", "tsi"], ["tma", "tal"], ["tex"], ["top", "tst"]],
    row: 1,
    ...
    startData() { return {
        ...
        domainIncrease: new Decimal(10),
        sinceCollapse: new Decimal(0),
        ...
        accumulationCap: new Decimal(50),
        ...
        simplification: new Decimal(0),
        simplificationEffect: new Decimal(1),
        simplifiers: {
            0: {
        ...
        exponentiateTotal: new Decimal(0),
        exponentiateReq: new Decimal(64),
        exponentiateGain: new Decimal(0),
        exponentiateEffect: new Decimal(1),

        // OPTIMIZATION
        optimization: new Decimal(0),
    ...
    automate() {
        if (player.tad.clickables[1010]) {
            player.tad.compressionMax = true
            if (player.tad.clickables[1011]) buyBuyable("tad", 101)
            if (player.tad.clickables[1012]) buyBuyable("tad", 102)
            if (player.tad.clickables[1013]) buyBuyable("tad", 103)
            if (player.tad.clickables[1014]) buyBuyable("tad", 105)
            if (player.tad.clickables[1015]) buyBuyable("tad", 106)
            if (player.tad.clickables[1016]) buyBuyable("tad", 104)
        }
        if (player.tad.clickables[1020]) {
            if (player.tad.autoExpand >= 1e25 && layers.tad.clickables[8].canClick()) clickClickable("tad", 8)
            else if (player.tad.autoExpand >= 1e5 && layers.tad.clickables[7].canClick()) clickClickable("tad", 7)
            else clickClickable("tad", 6)
        }
    },
    nodeStyle() {
        return {
    ...
    branches: ["ta", "ip", "om"],
    update(delta) {
        delta = Decimal.div(delta, player.uni["U2"].tickspeed)
        ...
        for (let i = 1; i < 5; i++) {
            if (hasUpgrade("tad", 103+(i*10))) {
                if (hasUpgrade("tad", 153)) player.tad.matterGain = player.tad.matterGain.mul(Decimal.add(Decimal.sumArithmeticSeries(getBuyableAmount("tad", 10+i), player.tad.accumulationMult, player.tad.accumulationMult.div(10), 0), 1))
                else player.tad.matterGain = player.tad.matterGain.mul(getBuyableAmount("tad", 10+i).mul(player.tad.accumulationMult).add(1))
            }
        }
        ...
        // ACCUMULATION PURCHASE CAP
        player.tad.accumulationCap = new Decimal(50)
        if (hasUpgrade("tad", 1)) player.tad.accumulationCap = player.tad.accumulationCap.add(50)
        ...
        player.tad.accumulationScale = player.tad.accumulationScale.mul(buyableEffect("tad", 501))
        ...
        player.tad.amplificationBase = player.tad.amplificationBase.mul(buyableEffect("tad", 105))
        ...
        player.tad.amplificationBase = player.tad.amplificationBase.mul(buyableEffect("tad", 502))
        ...
        player.tad.amplificationRaise = player.tad.amplificationRaise.mul(buyableEffect("tad", 309))
        ...
        if (hasMilestone("tad", 5)) player.tad.compressionMult = player.tad.compressionMult.mul(player.tad.magnification.max(15).sub(14).log(2).div(5).add(1))
        player.tad.compressionMult = player.tad.compressionMult.mul(buyableEffect("tad", 503))
        ...
        if (hasUpgrade("tad", 2)) player.tad.compressorCapAdd = player.tad.compressorCapAdd.add(10)

        // SIMPLIFICATION MODIFIERS
        for (let i = 0; i < 4; i++) {
            let base = new Decimal(5)
            if (i > 0) base = Decimal.mul(i, 10)
            player.tad.simplifiers[i].level = player.tad.simplifiers[i].amount.mul(21).mul(Decimal.div(21, 20).ln()).div(20).div(base).lambertw().div(Decimal.div(21, 20).ln()).floor()
            let pastReq = Decimal.pow(1.05, player.tad.simplifiers[i].level.sub(1)).mul(base).mul(player.tad.simplifiers[i].level).max(0)
            ...
            player.tad.simplifiers[i].effect = i == 0 ? player.tad.simplifiers[i].level : Decimal.pow(1.1, player.tad.simplifiers[i].level)
            if (getBuyableAmount("tad", 401+i).gt(0)) player.tad.simplifiers[i].amount = player.tad.simplifiers[i].amount.add(Decimal.mul(delta, divisor))
        }

        player.tad.simplification = player.tad.simplifiers[0].effect
        player.tad.simplificationEffect = Decimal.pow(10, player.tad.simplification.pow(0.5))
        ...
        magnificationDiv = magnificationDiv.mul(buyableEffect("tad", 505))
        ...
        player.tad.exponentiateReq = player.tad.exponentiateTotal.mul(16).add(64)

        let amt = player.tad.exponentiateTotal.gt(10) ? player.tad.exponentiateTotal.sub(10).pow(0.5).add(10) : player.tad.exponentiateTotal
        player.tad.exponentiateEffect = amt.div(100).add(1)
        ...
        if (player.tad.clickables[1004]) player.tad.autoAmplifier = player.tad.autoAmplifier.add(delta)
        if (player.tad.autoAmplifier.gte(Decimal.div(5, buyableEffect("tad", 1002)))) {
        ...
        // COLLAPSE CODE
        if (player.tad.matter.gte(player.tad.domainCap) && player.tad.clickables[1002]) {
            ...
            player.tad.sinceCollapse = new Decimal(0)
        ...
        if (hasUpgrade("tad", 145)) t1Mult = t1Mult.mul(1.2)
        ...
        if (hasUpgrade("tad", 145)) t2Mult = t2Mult.mul(1.2)
        ...
        if (hasUpgrade("tad", 145)) t3Mult = t3Mult.mul(1.2)
        ...
        let t2Auto = buyableEffect("tad", 1005).add(buyableEffect("tad", 1006)).sub(2)
        if (t2Auto.gt(0)) {
            player.tad.altInfinities["corrupted"].amount = player.tad.altInfinities["corrupted"].amount.add(player.tad.altInfinities["corrupted"].gain.mul(t2Auto).mul(delta))
            player.tad.altInfinities["disfigured"].amount = player.tad.altInfinities["disfigured"].amount.add(player.tad.altInfinities["disfigured"].gain.mul(t2Auto).mul(delta))
            player.tad.altInfinities["distorted"].amount = player.tad.altInfinities["distorted"].amount.add(player.tad.altInfinities["distorted"].gain.mul(t2Auto).mul(delta))
        }
    ...
    clickables: {
        ...
        6: {
            title: "x10",
            canClick() {return player.tad.highestCap.mul(player.tad.domainIncrease).gte(player.tad.domainCap.mul(10))},
            unlocked: true,
            tooltip() {
                if (this.canClick()) return ""
                return "Need to beat " + formatWhole(player.tad.highestCap.mul(player.tad.domainIncrease)) + " cap first!"
            },
            onClick() {
        ...
        7: {
            title: "x1e5",
            canClick() {return player.tad.highestCap.mul(player.tad.domainIncrease).gte(player.tad.domainCap.mul(1e5))},
            unlocked: true,
            tooltip() {
                if (this.canClick()) return ""
                return "Need to beat " + formatWhole(player.tad.highestCap.mul(player.tad.domainIncrease.mul(1e4))) + " cap first!"
            },
            onClick() {
        ...
        8: {
            title: "x1e25",
            canClick() {return player.tad.highestCap.mul(player.tad.domainIncrease).gte(player.tad.domainCap.mul(1e25))},
            unlocked: true,
            tooltip() {
                if (this.canClick()) return ""
                return "Need to beat " + formatWhole(player.tad.highestCap.mul(player.tad.domainIncrease.mul(1e24))) + " cap first!"
            },
            onClick() {
        ...
        12: {
            ...
            unlocked: true,
            onClick() {
                if (confirm("Are you sure you want to reset pre-compression content??")) {
            ...
            style() {
                let look = {width: "250px", minHeight: "40px", lineHeight: "0.9", border: "3px solid rgba(0,0,0,0.5)", borderRadius: "15px"}
                if (this.canClick()) {look.color = "white";look.background = "#094242"} else {look.color = "black";look.background = "#bf8f8f"}
                return look
            },
        },
        ...
        31: {
            title() {
                return "<h2>Magnify, but reset previous Domain content.</h2><br><h3>Req: " + format(player.tad.magnificationReq) + " Matter</h3>"
            },
        ...
        41: {
            title() {
                return "<h2>Exponentiate, but reset previous Domain content.</h2><br><h3>Req: " + formatWhole(player.tad.exponentiateReq) + " Magnifications</h3>"
            },
            canClick() { return player.tad.exponentiateGain.gte(1)},
            unlocked: true,
            onClick() {
                player.tad.exponentiate = player.tad.exponentiate.add(player.tad.exponentiateGain)
                player.tad.exponentiateTotal = player.tad.exponentiateTotal.add(player.tad.exponentiateGain)
                // RESET
        ...
        42: {
            title() { return "Respec your exponentiates<br><small style='font-size:11px'>(Does a exponentiation reset)</small>"},
            canClick() { return player.tad.exponentiateTotal.gt(player.tad.exponentiate)},
            unlocked: true,
            onClick() {
                if (confirm("Are you sure you want to reset pre-exponentiation content??")) {
                    player.tad.exponentiate = player.tad.exponentiateTotal
        ...
        51: {
            ...
            onClick() {
                ...
                player.tad.infinitum = player.tad.infinitum.sub(player.tad.optimizationReq)
            },
            style() {
                let look = {width: "400px", minHeight: "100px", border: "5px solid rgba(0,0,0,0.5)", borderRadius: "15px"}
        ...
        61: {
            title() {
                return "<h2>Alter, but reset previous Domain content.</h2><br><h3>Req: 250 Simplifications</h3>"
            },
            canClick() { return player.tad.simplification.gte(250)},
            unlocked: true,
            onClick() {
                player.tad.alteration = player.tad.alteration.add(player.tad.alterationGain)
                // RESET
                layers.tad.domainReset(3)
            },
            style() {
                let look = {width: "400px", minHeight: "100px", border: "5px solid rgba(0,0,0,0.5)", borderRadius: "15px"}
                if (this.canClick()) {look.background = "#927da7"} else {look.background = "#bf8f8f"}
                return look
            },
        },
        101: {
        ...
        1001: {
            title() {return player.tad.clickables[1001] ? "[ON]" : "[OFF]"},
            canClick: true,
            unlocked: true,
            onClick() {
                if (player.tad.clickables[1001]) {
                    player.tad.clickables[1001] = false
                } else {
                    player.tad.clickables[1001] = true
                }
            },
            style() {
                let look = {width: "250px", minHeight: "37px", color: "rgba(0,0,0,0.8)", fontSize: "14px", border: "3px solid rgba(0,0,0,0.2)", borderRadius: "0px"}
                if (player.tad.clickables[1001]) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
        1002: {
            title() {return player.tad.clickables[1002] ? "[ON]" : "[OFF]"},
            canClick: true,
            unlocked: true,
            onClick() {
                if (player.tad.clickables[1002]) {
                    player.tad.clickables[1002] = false
                } else {
                    player.tad.clickables[1002] = true
                }
            },
            style() {
                ...
                if (player.tad.clickables[1002]) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
        1003: {
            title() {return player.tad.clickables[1003] ? "[ON]" : "[OFF]"},
            canClick: true,
            unlocked: true,
            onClick() {
                if (player.tad.clickables[1003]) {
                    player.tad.clickables[1003] = false
                } else {
                    player.tad.clickables[1003] = true
                }
            },
            style() {
                let look = {width: "250px", minHeight: "37px", color: "rgba(0,0,0,0.8)", fontSize: "14px", border: "3px solid rgba(0,0,0,0.2)", borderRadius: "0 0 16px 16px"}
                if (player.tad.clickables[1003]) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
        1004: {
            title() {return player.tad.clickables[1004] ? "[ON]" : "[OFF]"},
            canClick: true,
            unlocked: true,
            onClick() {
                if (player.tad.clickables[1004]) {
                    player.tad.clickables[1004] = false
                } else {
                    player.tad.clickables[1004] = true
                }
            },
            style() {
                let look = {width: "250px", minHeight: "37px", color: "rgba(0,0,0,0.8)", fontSize: "14px", border: "3px solid rgba(0,0,0,0.2)", borderRadius: "0px"}
                if (player.tad.clickables[1004]) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
        1005: {
            title() {return player.tad.clickables[1005] ? "[ON]" : "[OFF]"},
            canClick: true,
            unlocked: true,
            onClick() {
                if (player.tad.clickables[1005]) {
                    player.tad.clickables[1005] = false
                } else {
                    player.tad.clickables[1005] = true
                }
            },
            style() {
                let look = {width: "250px", minHeight: "37px", color: "rgba(0,0,0,0.8)", fontSize: "14px", border: "3px solid rgba(0,0,0,0.2)", borderRadius: "0px"}
                if (player.tad.clickables[1005]) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
        1006: {
            title() {return player.tad.clickables[1006] ? "Disable Mag. Popups<br><small>[ON]</small>" : "Disable Mag. Popups<br><small>[OFF]</small>"},
            canClick: true,
            unlocked: true,
            onClick() {
                if (player.tad.clickables[1006]) {
                    player.tad.clickables[1006] = false
                } else {
                    player.tad.clickables[1006] = true
                }
            },
            style() {
        ...
        1010: {
            title() {return player.tad.clickables[1010] ? "[ON]" : "[OFF]"},
            canClick: true,
            unlocked: true,
            onClick() {
                if (player.tad.clickables[1010]) {
                    player.tad.clickables[1010] = false
                } else {
                    player.tad.clickables[1010] = true
                }
            },
            style() {
                let look = {width: "250px", minHeight: "37px", color: "rgba(0,0,0,0.8)", fontSize: "14px", border: "3px solid rgba(0,0,0,0.2)", borderRadius: "0px"}
                if (player.tad.clickables[1010]) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
        1011: {
            title() {return player.tad.clickables[1011] ? "Ma:C<br><small>[ON]</small>" : "Ma:C<br><small>[OFF]</small>"},
            canClick: true,
            unlocked: true,
            onClick() {
                if (player.tad.clickables[1011]) {
                    player.tad.clickables[1011] = false
                } else {
                    player.tad.clickables[1011] = true
                }
            },
            style() {
                let look = {width: "83px", minHeight: "40px", color: "rgba(0,0,0,0.8)", lineHeight: "1", border: "3px solid rgba(0,0,0,0.2)", borderRadius: "0px"}
                if (player.tad.clickables[1011]) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
        1012: {
            title() {return player.tad.clickables[1012] ? "AC:C<br><small>[ON]</small>" : "AC:C<br><small>[OFF]</small>"},
            canClick: true,
            unlocked: true,
            onClick() {
                if (player.tad.clickables[1012]) {
                    player.tad.clickables[1012] = false
                } else {
                    player.tad.clickables[1012] = true
                }
            },
            style() {
                let look = {width: "84px", minHeight: "40px", color: "rgba(0,0,0,0.8)", lineHeight: "1", border: "3px solid rgba(0,0,0,0.2)", borderRadius: "0px"}
                if (player.tad.clickables[1012]) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
        1013: {
            title() {return player.tad.clickables[1013] ? "AE:C<br><small>[ON]</small>" : "AE:C<br><small>[OFF]</small>"},
            canClick: true,
            unlocked: true,
            onClick() {
                if (player.tad.clickables[1013]) {
                    player.tad.clickables[1013] = false
                } else {
                    player.tad.clickables[1013] = true
                }
            },
            style() {
                let look = {width: "83px", minHeight: "40px", color: "rgba(0,0,0,0.8)", lineHeight: "1", border: "3px solid rgba(0,0,0,0.2)", borderRadius: "0px"}
                if (player.tad.clickables[1013]) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
        1014: {
            title() {return player.tad.clickables[1014] ? "AB:C<br><small>[ON]</small>" : "AB:C<br><small>[OFF]</small>"},
            canClick: true,
            unlocked: true,
            onClick() {
                if (player.tad.clickables[1014]) {
                    player.tad.clickables[1014] = false
                } else {
                    player.tad.clickables[1014] = true
                }
            },
            style() {
                let look = {width: "83px", minHeight: "40px", color: "rgba(0,0,0,0.8)", lineHeight: "1", border: "3px solid rgba(0,0,0,0.2)", borderRadius: "0px"}
                if (player.tad.clickables[1014]) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
        1015: {
            title() {return player.tad.clickables[1015] ? "Co:C<br><small>[ON]</small>" : "Co:C<br><small>[OFF]</small>"},
            canClick: true,
            unlocked: true,
            onClick() {
                if (player.tad.clickables[1015]) {
                    player.tad.clickables[1015] = false
                } else {
                    player.tad.clickables[1015] = true
                }
            },
            style() {
                let look = {width: "84px", minHeight: "40px", color: "rgba(0,0,0,0.8)", lineHeight: "1", border: "3px solid rgba(0,0,0,0.2)", borderRadius: "0px"}
                if (player.tad.clickables[1015]) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
        1016: {
            title() {return player.tad.clickables[1016] ? "In:C<br><small>[ON]</small>" : "In:C<br><small>[OFF]</small>"},
            canClick: true,
            unlocked: true,
            onClick() {
                if (player.tad.clickables[1016]) {
                    player.tad.clickables[1016] = false
                } else {
                    player.tad.clickables[1016] = true
                }
            },
            style() {
                let look = {width: "83px", minHeight: "40px", color: "rgba(0,0,0,0.8)", lineHeight: "1", border: "3px solid rgba(0,0,0,0.2)", borderRadius: "0px"}
                if (player.tad.clickables[1016]) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
        1020: {
            title() {return player.tad.clickables[1020] ? "[ON]" : "[OFF]"},
            canClick: true,
            unlocked: true,
            onClick() {
                if (player.tad.clickables[1020]) {
                    player.tad.clickables[1020] = false
                } else {
                    player.tad.clickables[1020] = true
                }
            },
            style() {
                let look = {width: "250px", minHeight: "37px", color: "rgba(0,0,0,0.8)", fontSize: "14px", border: "3px solid rgba(0,0,0,0.2)", borderRadius: "0px"}
                if (player.tad.clickables[1020]) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
        1021: {
            title: "x10",
            canClick() {return player.tad.autoExpand != 10},
            unlocked: true,
            onClick() {
                player.tad.autoExpand = 10
            },
            style() {
                let look = {width: "83px", minHeight: "37px", color: "rgba(0,0,0,0.8)", border: "3px solid rgba(0,0,0,0.2)", borderRadius: "0 0 0 16px"}
                if (this.canClick()) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
        1022: {
            title: "x1e5",
            canClick() {return player.tad.autoExpand != 1e5},
            unlocked: true,
            onClick() {
                player.tad.autoExpand = 1e5
            },
            style() {
                let look = {width: "84px", minHeight: "37px", color: "rgba(0,0,0,0.8)", border: "3px solid rgba(0,0,0,0.2)", borderRadius: "0"}
                if (this.canClick()) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
        1023: {
            title: "x1e25",
            canClick() {return player.tad.autoExpand != 1e25},
            unlocked: true,
            onClick() {
                player.tad.autoExpand = 1e25
            },
            style() {
                let look = {width: "83px", minHeight: "37px", color: "rgba(0,0,0,0.8)", border: "3px solid rgba(0,0,0,0.2)", borderRadius: "0 0 16px 0"}
                if (this.canClick()) {look.backgroundColor = "#964c4a"} else {look.backgroundColor = "#743b3a"}
                return look
            },
        },
    ...
    upgrades: {
        1: {
            title: "Exponentiate (1:1)",
            unlocked: true,
            description: "Increase accumulator caps by +50<br><small>[Costs scale exponentially past 50]</small>",
            cost: new Decimal(1),
            currencyLocation() {return player.tad},
            currencyDisplayName: "Exponentiation",
            currencyInternalName: "exponentiate",
            style() {
                let look = {width: "130px", minHeight: "110px", lineHeight: "1", color: "rgba(0,0,0,0.8)", border: "3px solid rgba(0,0,0,0.5)", borderRadius: "15px", margin: "2px"}
                hasUpgrade(this.layer, this.id) ? look.background = "#77bf5f" : !canAffordUpgrade(this.layer, this.id) ? look.background = "#bf8f8f" : look.background = "#ffd5b3"
                return look
            },
        },
        2: {
            title: "Exponentiate (1:2)",
            unlocked: true,
            description: "Increase compressor cap by +10",
            cost: new Decimal(1),
            currencyLocation() {return player.tad},
            currencyDisplayName: "Exponentiation",
            currencyInternalName: "exponentiate",
            style() {
                let look = {width: "130px", minHeight: "110px", color: "rgba(0,0,0,0.8)", border: "3px solid rgba(0,0,0,0.5)", borderRadius: "15px", margin: "2px"}
                hasUpgrade(this.layer, this.id) ? look.background = "#77bf5f" : !canAffordUpgrade(this.layer, this.id) ? look.background = "#bf8f8f" : look.background = "#ffd5b3"
                return look
            },
        },
        ...
        136: {
            title: "Infinitum (3:3)",
            unlocked() {return hasUpgrade("tad", 125)},
            description() {return "Total compressions increase amplifier base."},
            cost: new Decimal(16),
            currencyLocation() { return player.tad },
            currencyDisplayName: "Infinitum",
            currencyInternalName: "infinitum",
            effect() {
                return player.tad.compressionTotal.pow(0.7).div(10).add(1)
            },
        ...
        146: {
            title: "Infinitum (4:3)",
            unlocked() {return hasUpgrade("bi", 16) && hasUpgrade("tad", 135)},
            description() {return "Time since domain collapse boosts amplifier base."},
            cost: new Decimal(7500),
            currencyLocation() { return player.tad },
            currencyDisplayName: "Infinitum",
            currencyInternalName: "infinitum",
            effect() {
                return player.tad.sinceCollapse.add(1).log(3).div(10).add(1)
            },
            effectDisplay() { return "x" + formatSimple(upgradeEffect(this.layer, this.id), 2) }, // Add formatting to the effect
            style() {
                let look = {width: "130px", minHeight: "110px", color: "rgba(0,0,0,0.8)", border: "3px solid rgba(0,0,0,0.5)", borderRadius: "15px", margin: "2px"}
        ...
        153: {
            title: "Infinitum (5:4)",
            unlocked() {return player.al.cocoonLevel >= 5 && hasUpgrade("tad", 145)},
            description() {return "Improve accumulator second effect formula."},
        ...
        156: {
            title: "Infinitum (5:3)",
            unlocked() {return player.al.cocoonLevel >= 5 && hasUpgrade("tad", 145)},
            description() {return "Reduce simplifier cooldowns based on magnifications."},
            cost: new Decimal(3e12),
            currencyLocation() { return player.tad },
            currencyDisplayName: "Infinitum",
            currencyInternalName: "infinitum",
            effect() {
                return player.tad.magnification.div(100).add(1)
            },
    ...
    buyables: {
        11: {
            ...
            purchaseLimit() {return player.tad.accumulationCap},
            currency() { return player.tad.matter},
            pay(amt) { player.tad.matter = this.currency().sub(amt) },
            effect(x) {
                return getBuyableAmount(this.layer, this.id).mul(Decimal.pow(player.tad.accumulationScale, getBuyableAmount(this.layer, this.id)))
            },
            unlocked: true,
            cost(x) {
                let amt = x || getBuyableAmount(this.layer, this.id)
                if (amt.gt(50)) amt = amt.sub(50).pow(2).add(50)
                return this.costGrowth().pow(amt).mul(this.costBase())
            },
            canAfford() { return this.currency().gte(this.cost()) },
            display() {
                if (hasUpgrade("tad", 113)) {
                    let eff2 = getBuyableAmount(this.layer, this.id).mul(player.tad.accumulationMult).add(1)
                    if (hasUpgrade("tad", 153)) eff2 = Decimal.add(Decimal.sumArithmeticSeries(getBuyableAmount(this.layer, this.id), player.tad.accumulationMult, player.tad.accumulationMult.div(10), 0), 1)
                    return "<h3>Accumulator [1:1]\n\
                        ...
                        Multiplies matter gain by x" + formatSimple(eff2, 2) + ".\n\
            ...
            buy() {
                ...
                if (!player.tad.accumulationMax) {
                    let buyonecost = new Decimal(this.costGrowth()).pow(amt).mul(this.costBase())
                    if (player.tad.optimization.lt(3)) this.pay(buyonecost)

                    setBuyableAmount(this.layer, this.id, getBuyableAmount(this.layer, this.id).add(1))
                } else {
                    let max = Decimal.affordGeometricSeries(this.currency(), this.costBase(), this.costGrowth(), getBuyableAmount(this.layer, this.id)), purmax = max
                    let relativeLimit = this.purchaseLimit()
                    if (relativeLimit.gt(50)) relativeLimit = relativeLimit.sub(50).pow(2).add(50)
                    let cost = Decimal.sumGeometricSeries(max.min(relativeLimit.sub(amt)), this.costBase(), this.costGrowth(), amt)
                    if (player.tad.optimization.lt(3)) this.pay(cost)

                    if (purmax.add(getBuyableAmount(this.layer, this.id)).gt(50)) purmax = purmax.add(getBuyableAmount(this.layer, this.id)).sub(50).pow(0.5).add(50).sub(getBuyableAmount(this.layer, this.id)).add(1)
                    if (purmax.gt(this.purchaseLimit().sub(getBuyableAmount(this.layer, this.id)))) purmax = this.purchaseLimit().sub(getBuyableAmount(this.layer, this.id))
                    setBuyableAmount(this.layer, this.id, getBuyableAmount(this.layer, this.id).add(purmax.floor()))
                }
            },
            style() {
        ...
        12: {
            ...
            pay(amt) { player.tad.matter = this.currency().sub(amt) },
            effect(x) {
                return getBuyableAmount(this.layer, this.id).mul(4).mul(Decimal.pow(player.tad.accumulationScale, getBuyableAmount(this.layer, this.id)))
            },
            unlocked: true,
            cost(x) {
                let amt = x || getBuyableAmount(this.layer, this.id)
        ...
        13: {
            ...
            pay(amt) { player.tad.matter = this.currency().sub(amt) },
            effect(x) {
                return getBuyableAmount(this.layer, this.id).mul(400).mul(Decimal.pow(player.tad.accumulationScale, getBuyableAmount(this.layer, this.id)))
            },
        ...
        14: {
            ...
            pay(amt) { player.tad.matter = this.currency().sub(amt) },
            effect(x) {
                return getBuyableAmount(this.layer, this.id).mul(1.6e6).mul(Decimal.pow(player.tad.accumulationScale, getBuyableAmount(this.layer, this.id)))
            },
        ...
        21: {
            ...
            display() {
                return "<h3>Accumulator [2:1]\n\
                    (" + formatWhole(getBuyableAmount(this.layer, this.id)) + "/" + formatWhole(this.purchaseLimit()) + ")</h3>\n\
                    Increases base matter gain by +" + formatSimple(tmp[this.layer].buyables[this.id].effect, 1) + ".\n\
                    Cost: " + formatSimple(tmp[this.layer].buyables[this.id].cost, 1) + " Matter"
            },
            buy() {
        ...
        101: {
            ...
            style() {
                let look = {width: "200px", height: "80px", border: "3px solid rgba(0,0,0,0.3)", borderRadius: "15px", margin: "3px"}
        ...
        105: {
            ...
            pay(amt) { player.tad.compression = this.currency().sub(amt) },
            effect(x) {
                let base = new Decimal(0.5).mul(player.tad.compressionMult).add(1)
                return Decimal.pow(base, getBuyableAmount(this.layer, this.id))
            },
            unlocked: true,
            cost(x) { return (x || getBuyableAmount(this.layer, this.id)).add(1) },
            canAfford() { return this.currency().gte(this.cost()) },
            display() {
                return "<h3>Ampli-Base Compressor</h3><br>(" + formatWhole(getBuyableAmount(this.layer, this.id)) + "/" + formatWhole(this.purchaseLimit()) + ")\n\
                    x" + formatSimple(tmp[this.layer].buyables[this.id].effect, 2) + " Amplifier Base<br>\n\
                    Cost: " + formatWhole(tmp[this.layer].buyables[this.id].cost) + " Compression"
            },
            buy() {
                if (!player.tad.compressionMax) {
        ...
        106: {
            ...
            canAfford() { return this.currency().gte(this.cost()) },
            display() {
                return "<h3>Compressor Compressor</h3><br><small>[UNEFFECTED BY COMPRESSOR BUFFS]</small><br>(" + formatWhole(getBuyableAmount(this.layer, this.id)) + "/" + formatWhole(this.purchaseLimit()) + ")\n\
                    x" + formatSimple(tmp[this.layer].buyables[this.id].effect, 2) + " Compressor Effectiveness<br>\n\
                    Cost: " + formatWhole(tmp[this.layer].buyables[this.id].cost) + " Compression"
            },
            buy() {
                if (!player.tad.compressionMax) {
        ...
        301: {
            ...
            buy(mult) {
                if (mult != true) {
                    let buyonecost = new Decimal(this.costGrowth()).pow(getBuyableAmount(this.layer, this.id)).mul(this.costBase())
                    if (player.tad.optimization.lt(7)) this.pay(buyonecost)
                    ...
                    if (max.gt(this.purchaseLimit().sub(getBuyableAmount(this.layer, this.id)))) { max = this.purchaseLimit().sub(getBuyableAmount(this.layer, this.id)) }
                    let cost = Decimal.sumGeometricSeries(max, this.costBase(), this.costGrowth(), getBuyableAmount(this.layer, this.id))
                    if (player.tad.optimization.lt(7)) this.pay(cost)

                    setBuyableAmount(this.layer, this.id, getBuyableAmount(this.layer, this.id).add(max))
                }
            },
            style: { width: '200px', height: '150px', color: "white", backgroundColor: '#446b3f', margin: "2px"}
        },
        302: {
        ...
        307: {
            ...
            unlocked() { return hasUpgrade("depth1", 103) },
            cost(x) { return this.costGrowth().pow(x || getBuyableAmount(this.layer, this.id)).mul(this.costBase()) },
            canAfford() { return this.currency().gte(this.cost()) },
            title() {
                return "Super Amplifier 1"
            },
            display() {
                return "which raises amplifier effects by ^" + formatSimple(tmp[this.layer].buyables[this.id].effect, 2) + ".\n\
                    Cost: " + format(tmp[this.layer].buyables[this.id].cost) + " Matter"
            },
            buy(mult) {
                if (mult != true) {
            ...
            style: { width: '200px', height: '150px', color: "black", backgroundColor: '#8fba89', margin: "2px"}
        },
        308: {
        ...
        309: {
            costBase() { return new Decimal(1e81) },
        ...
        401: {
            ...
            currency() { return player.tad.matter},
            pay(amt) { player.tad.matter = this.currency().sub(amt) },
            effect(x) { return Decimal.pow(1.1, getBuyableAmount(this.layer, this.id).sub(1)).max(1) },
            unlocked: true,
            cost(x) { return this.costGrowth().pow(x || getBuyableAmount(this.layer, this.id)).mul(this.costBase()) },
            canAfford() { return this.currency().gte(this.cost()) },
            display() {
                if (getBuyableAmount(this.layer, this.id).lte(0)) {
                    return "Unlock 1st Simplifier.\n\
                        Cost: " + format(tmp[this.layer].buyables[this.id].cost) + " Matter"
                }
            ...
            style: { width: '250px', height: '120px', color: "rgba(0,0,0,0.8)", backgroundColor: "#88655B", fontSize: "12px", boxSizing: "border-box"},
        },
        402: {
        ...
        403: {
            costBase() { return new Decimal(1e250) },
        ...
        404: {
            costBase() { return new Decimal("1e1500")},
            costGrowth() { return new Decimal(1e8)},
            purchaseLimit() { return new Decimal(250)},
            currency() { return player.tad.matter},
            pay(amt) { player.tad.matter = this.currency().sub(amt) },
        ...
        501: {
            ...
            purchaseLimit() { return new Decimal(100) },
            currency() { return player.tad.alteration},
            pay(amt) { player.tad.alteration = this.currency().sub(amt) },
            effect(x) {return getBuyableAmount(this.layer, this.id).div(20).add(1)},
            unlocked() { return true },
            cost(x) { return this.costGrowth().pow(x || getBuyableAmount(this.layer, this.id)).mul(this.costBase()).mul(10).floor().div(10) },
            canAfford() { return this.currency().gte(this.cost()) },
            title() {
                return "Accumulator Alterator"
            },
            display() {
                return "which multiplies accumulator effect scaling by x" + formatSimple(tmp[this.layer].buyables[this.id].effect, 2) + ".\n\
                    Cost: " + format(tmp[this.layer].buyables[this.id].cost) + " Alteration"
            },
            buy(mult) {
                if (mult != true) {
            ...
            style: { width: '200px', height: '150px', color: "black", backgroundColor: '#927da7', margin: "2px"}
        },
        502: {
            ...
            canAfford() { return this.currency().gte(this.cost()) },
            title() {
                return "Amplifier Alterator"
            },
            display() {
                return "which multiplies amplifier base by x" + formatSimple(tmp[this.layer].buyables[this.id].effect) + ".\n\
                    Cost: " + format(tmp[this.layer].buyables[this.id].cost) + " Alteration"
            },
            buy(mult) {
                if (mult != true) {
        ...
        503: {
            ...
            canAfford() { return this.currency().gte(this.cost()) },
            title() {
                return "Compressor Alterator"
            },
            display() {
                return "which multiplies compressor effectiveness by x" + formatSimple(tmp[this.layer].buyables[this.id].effect, 2) + ".\n\
                    Cost: " + format(tmp[this.layer].buyables[this.id].cost) + " Alteration"
            },
            buy(mult) {
                if (mult != true) {
        ...
        504: {
            ...
            canAfford() { return this.currency().gte(this.cost()) },
            title() {
                return "Simplifier Alterator"
            },
            display() {
                return "which divides simplifier cooldowns by /" + formatSimple(tmp[this.layer].buyables[this.id].effect, 2) + ".\n\
                    Cost: " + format(tmp[this.layer].buyables[this.id].cost) + " Alteration"
            },
            buy(mult) {
                if (mult != true) {
        ...
        505: {
            costBase() { return new Decimal(1296) },
            ...
            canAfford() { return this.currency().gte(this.cost()) },
            title() {
                return "Magnification Alterator"
            },
            display() {
                return "which divides magnification cost scaling by /" + formatSimple(tmp[this.layer].buyables[this.id].effect, 2) + ".\n\
                    Cost: " + format(tmp[this.layer].buyables[this.id].cost) + " Alteration"
            },
            buy(mult) {
                if (mult != true) {
        ...
        506: {
            costBase() { return new Decimal(16807) },
            ...
            canAfford() { return this.currency().gte(this.cost()) },
            title() {
                return "Alteration Alterator"
            },
            display() {
                return "which multiplies alteration gain by x" + formatSimple(tmp[this.layer].buyables[this.id].effect, 2) + ".\n\
                    Cost: " + format(tmp[this.layer].buyables[this.id].cost) + " Alteration"
            },
            buy(mult) {
                if (mult != true) {
        ...
        1005: {
            ...
            canAfford() { return this.currency().gte(this.cost()) },
            display() {
                return "Increase T2-I gain by +" + formatSimple(tmp[this.layer].buyables[this.id].effect.sub(1).mul(100)) + "%/s.\n\
                    Cost: " + format(tmp[this.layer].buyables[this.id].cost) + " Infinitum"
            },
            buy(mult) {
                if (!mult) {
    ...
    bars: {
        autoAccumulator: {
            ...
            height: 42,
            progress() {
                return player.tad.autoAccumulator.div(Decimal.div(5, buyableEffect("tad", 1001)))
            },
            ...
            display() {
                return formatTime(player.tad.autoAccumulator) + "/" + formatTime(Decimal.div(5, buyableEffect("tad", 1001)))
            },
        },
        autoAmplifier: {
            unlocked: true,
            ...
            height: 42,
            progress() {
                return player.tad.autoAmplifier.div(Decimal.div(5, buyableEffect("tad", 1002)))
            },
            baseStyle: {backgroundColor: "#321918"},
            fillStyle: {backgroundColor: "#854442"},
            borderStyle: {border: "0px solid #532a29", borderBottom: "4px solid #532a29", borderRadius: "0"},
        ...
        simplifier1: {
            unlocked() {return getBuyableAmount("tad", 401).gt(0)},
            direction: RIGHT,
            width: 250,
            height: 40,
            progress() {
                return player.tad.simplifiers[0].current.div(player.tad.simplifiers[0].next)
            },
            ...
            textStyle: {color: "rgba(255,255,255,0.8)", fontSize: "16px", fontFamily: "monospace"},
            display() {
                return formatTime(player.tad.simplifiers[0].current) + "/" + formatTime(player.tad.simplifiers[0].next)
            },
        },
        simplifier2: {
            unlocked() {return getBuyableAmount("tad", 402).gt(0)},
            direction: RIGHT,
            width: 250,
            height: 40,
            progress() {
                return player.tad.simplifiers[1].current.div(player.tad.simplifiers[1].next)
            },
            baseStyle: {backgroundColor: "#281e1b"},
            fillStyle: {backgroundColor: "#6c5048"},
            borderStyle: {border: "0px solid #44322d", borderBottom: "3px solid #44322d", borderRadius: "0"},
    ...
    milestones: {
        1: {
            ...
            unlocked: true,
            disablePopup() {return player.tad.clickables[1006]},
            color: "#6d228b",
        ...
        5: {
            ...
            effectDescription() { return "Multiply compressor effect base.<br>Currently: x" + formatSimple(player.tad.magnification.max(15).sub(14).log(2).div(5).add(1)) },
        ...
        7: {
            ...
            effectDescription() { return "Raise amplifier effects.<br>Currently: ^" + formatSimple(player.tad.magnification.max(64).sub(63).pow(0.5).div(50).add(1), 3) },
    ...
    domainReset(tier = 0) {
        ...
        // SIMPLIFICATIONS
        if (tier > 2) {
            player.tad.simplification = new Decimal(0)
            player.tad.simplificationEffect = new Decimal(1)
            for (let i = 0; i < 4; i ++) {
                player.tad.simplifiers[i].amount = new Decimal(0)
                player.tad.simplifiers[i].current = new Decimal(0)
                player.tad.simplifiers[i].next = new Decimal(30)
                player.tad.simplifiers[i].level = new Decimal(0)
                player.tad.simplifiers[i].effect = i == 0 ? new Decimal(0) : new Decimal(1)
            }
        ...
        // EXPONENTIATIONS
        if (tier > 4) {
            player.tad.exponentiate = new Decimal(0)
            player.tad.exponentiateGain = new Decimal(0)
            for (let i = 0; i < player.tad.upgrades.length; i++) {
                if (+player.tad.upgrades[i] < 100) {
                    player.tad.upgrades.splice(i, 1);
                    i--;
        ...
        Tabs: {
            ...
            "Infinitum": {
                ...
                content: [
                    ...
                    ["row", [["upgrade", 131], ["upgrade", 132], ["upgrade", 136], ["upgrade", 133], ["upgrade", 134], ["upgrade", 135]]],
                    ["row", [["upgrade", 141], ["upgrade", 142], ["upgrade", 146], ["upgrade", 143], ["upgrade", 144], ["upgrade", 145]]],
                    ["row", [["upgrade", 151], ["upgrade", 152], ["upgrade", 156], ["upgrade", 153], ["upgrade", 154], ["upgrade", 155]]],
                    ["blank", "25px"],
                    ["style-column", [
                        ["style-column", [
                        ...
                        ["style-column", [
                            ...
                            ["style-column", [
                                ["style-row", [
                                    ...
                                    ["style-row", [
                                        ["color-text", [() => {return "x" + formatSimple(player.tad.altInfinities.corrupted.effect1, 2) + " Amplifier Base."}, true, "black", () => {return player.tad.altInfinities.corrupted.milestone.gte(1)}, "rgba(0,0,0,0.5)"]],
                                    ], {width: "207px", height: "40px"}],
                                ...
                                ["style-row", [
                                    ...
                                    ["style-row", [
                                        ["color-text", [() => {return "Unlock Magnification Layer."}, true, "black", () => {return player.tad.altInfinities.corrupted.milestone.gte(3)}, "rgba(0,0,0,0.5)"]],
                                    ], {width: "207px", height: "40px"}],
                                ], {width: "250px", height: "40px"}],
                        ...
                        ["style-column", [
                            ...
                            ["style-column", [
                                ...
                                ["style-row", [
                                    ...
                                    ["style-row", [
                                        ["color-text", [() => {return "Unlock Optimization Layer."}, true, "black", () => {return player.tad.altInfinities.disfigured.milestone.gte(3)}, "rgba(0,0,0,0.5)"]],
                                    ], {width: "207px", height: "40px"}],
                                ], {width: "250px", height: "40px"}],
                        ...
                        ["style-column", [
                            ...
                            ["style-column", [
                                ...
                                ["style-row", [
                                    ...
                                    ["style-row", [
                                        ["color-text", [() => {return "Unlock Simplification Layer"}, true, "black", () => {return player.tad.altInfinities.distorted.milestone.gte(3)}, "rgba(0,0,0,0.5)"]],
                                    ], {width: "207px", height: "40px"}],
                                ], {width: "250px", height: "40px"}],
                        ...
                        ["style-column", [
                            ...
                            ["style-column", [
                                ["style-row", [
                                    ...
                                    ["style-row", [
                                        ["color-text", [() => {return "/" + formatSimple(player.tad.altInfinities.infected.effect1, 2) + " Magnification Req."}, true, "black", () => {return player.tad.altInfinities.infected.milestone.gte(1)}, "rgba(0,0,0,0.5)"]],
                                    ], {width: "207px", height: "40px", lineHeight: "1"}],
                        ...
                        ["style-column", [
                            ...
                            ["style-column", [
                                ...
                                ["style-row", [
                                    ...
                                    ["style-row", [
                                        ["color-text", [() => {return "Unlock Alteration Layer"}, true, "black", () => {return player.tad.altInfinities.infested.milestone.gte(3)}, "rgba(0,0,0,0.5)"]],
                                    ], {width: "207px", height: "40px"}],
                                ], {width: "250px", height: "40px"}],
            ...
            "Perks": {
                buttonStyle() { return { color: "black", borderColor: "#008080", borderRadius: "5px" } },
                unlocked() { return player.in.unlockedBreak },
                content: [
                    ["blank", "25px"],
                    ["style-column", [
                        ["raw-html", "Perks for defeating Tav", {color: "rgba(0,0,0,0.7)", fontSize: "24px", fontFamily: "monospace"}],
                    ], {width: "800px", border: "3px solid #008080", backgroundImage: "linear-gradient(45deg, #31aeb0 0%, #b2d8d8 100%)", borderBottom: "5px", paddingTop: "5px", paddingBottom: "5px", borderRadius: "15px 15px 0px 0px"}],
                    ["style-column", [
                        ["raw-html", "<u>Unlocks</u>", {color: "rgba(0,0,0,0.7)", fontSize: "20px", fontFamily: "monospace"}],
                        ["raw-html", "Break Infinity OTF.", {color: "rgba(0,0,0,0.7)", fontSize: "18px", fontFamily: "monospace"}],
                        ["raw-html", "Break Infinity layer.", {color: "rgba(0,0,0,0.7)", fontSize: "18px", fontFamily: "monospace"}],
                        ["raw-html", "New checkback evolutions.", {color: "rgba(0,0,0,0.7)", fontSize: "18px", fontFamily: "monospace"}],
                        ["raw-html", "New infinity milestones.", {color: "rgba(0,0,0,0.7)", fontSize: "18px", fontFamily: "monospace"}],
                        ["raw-html", "A new origin upgrade.", {color: "rgba(0,0,0,0.7)", fontSize: "18px", fontFamily: "monospace"}],
                        ["raw-html", "<u>Buffs</u>", {color: "rgba(0,0,0,0.7)", fontSize: "20px", fontFamily: "monospace"}],
                        ["raw-html", () => {return "x1.5 Dice Points, Rocket Fuel, and " + player.h.stageName[0] + " Points."}, {color: "rgba(0,0,0,0.7)", fontSize: "18px", fontFamily: "monospace"}],
                        ["raw-html", "x1.25 Infinities, IP, and NIP.", {color: "rgba(0,0,0,0.7)", fontSize: "18px", fontFamily: "monospace"}],
                        ["raw-html", "x1.2 Checkback XP.", {color: "rgba(0,0,0,0.7)", fontSize: "18px", fontFamily: "monospace"}],
                        ["raw-html", "/1.1 Crate Button Cooldowns.", {color: "rgba(0,0,0,0.7)", fontSize: "18px", fontFamily: "monospace"}],
                        ["raw-html", "+0.1 Evolution Shard Chance.", {color: "rgba(0,0,0,0.7)", fontSize: "18px", fontFamily: "monospace"}],
                    ], {width: "800px", border: "3px solid #008080", backgroundImage: "linear-gradient(45deg, #31aeb0 0%, #b2d8d8 100%)", paddingTop: "5px", paddingBottom: "5px", borderRadius: "0px 0px 15px 15px"}]
                ]
            },
        },
        Domain: {
            ...
            "Amplification": {
                ...
                content: [
                    ...
                    ["blank", "15px"],
                    ["row", [["rounded-ex-buyable", 301], ["rounded-ex-buyable", 302], ["rounded-ex-buyable", 303]]],
                    ["row", [["rounded-ex-buyable", 304], ["rounded-ex-buyable", 305], ["rounded-ex-buyable", 306]]],
                    ["style-row", [
                        ["rounded-ex-buyable", 307], ["rounded-ex-buyable", 308], ["rounded-ex-buyable", 309]
                    ], () => {return hasUpgrade("depth1", 103) ? {marginTop: "10px"} : {display: "none !important"}}],
                ]
            },
            "Compression": {
                buttonStyle() { return { color: "black", borderRadius: "5px" }},
                ...
                content: [
                    ...
                    ["row", [["buyable", 105], ["buyable", 106], ["buyable", 104]]],
                ]
            },
            "Simplification": {
                buttonStyle() { return { color: "black", borderRadius: "5px" }},
                unlocked() { return true },
                content: [
                    ["raw-html", () => {return player.tad.simplification.neq(1) ? "You have <h3>" + formatWhole(player.tad.simplification) + "</h3> simplifications." : "You have <h3>" + formatWhole(player.tad.simplification) + "</h3> simplification." }, {color: "black", fontSize: "24px", fontFamily: "monospace"}],
                    ["raw-html", () => {return "Your simplifications reduce compression requirement by /" + formatWhole(player.tad.simplificationEffect)}, {color: "black", fontSize: "20px", fontFamily: "monospace"}],
                    ["blank", "20px"],
                    ["style-row", [
                        ["top-column", [
                            ["style-column", [
                                ["raw-html", () => {return getBuyableAmount("tad", 401).gt(0) ? "<h3>1st Simplifier</h3><hr style='border:1px solid #44322d'>Gives +" + formatSimple(player.tad.simplifiers[0].effect) + " Simplifications" : ""}, {color: "rgba(0,0,0,0.8)", fontSize: "14px", fontFamily: "monospace"}],
                            ], () => {return getBuyableAmount("tad", 401).gt(0) ? {width: "250px", height: "60px", lineHeight: "1", borderBottom: "4px solid #44322d"} : {width: "250px", height: "20px", borderBottom: "4px solid #44322d"}}],
                            ["bar", "simplifier1"],
                            ["ex-buyable", 401],
                            ["style-row", [], {width: "250px", height: "20px", borderTop: "3px solid #44322d"}],
                        ], {width: "250px", background: "#88655B", border: "4px solid #44322d", borderRadius: "20px", margin: "5px"}],
                        ["top-column", [
                            ["style-column", [
                                ["raw-html", () => {return getBuyableAmount("tad", 402).gt(0) ? "<h3>2nd Simplifier</h3><hr style='border:1px solid #44322d'>Reduces 1st Simplifier cooldown by /" + formatSimple(player.tad.simplifiers[1].effect) : ""}, {color: "rgba(0,0,0,0.8)", fontSize: "14px", fontFamily: "monospace"}],
                            ], () => {return getBuyableAmount("tad", 402).gt(0) ? {width: "250px", height: "60px", lineHeight: "1", borderBottom: "4px solid #44322d"} : {width: "250px", height: "20px", borderBottom: "4px solid #44322d"}}],
                            ["bar", "simplifier2"],
                            ["ex-buyable", 402],
                            ["style-row", [], {width: "250px", height: "20px", borderTop: "3px solid #44322d"}],
                        ], {width: "250px", background: "#88655B", border: "4px solid #44322d", borderRadius: "20px", margin: "5px"}],
                        ["top-column", [
                            ["style-column", [
                                ["raw-html", () => {return getBuyableAmount("tad", 403).gt(0) ? "<h3>3rd Simplifier</h3><hr style='border:1px solid #44322d'>Reduces 2nd Simplifier cooldown by /" + formatSimple(player.tad.simplifiers[2].effect) : ""}, {color: "rgba(0,0,0,0.8)", fontSize: "14px", fontFamily: "monospace"}],
                            ], () => {return getBuyableAmount("tad", 403).gt(0) ? {width: "250px", height: "60px", lineHeight: "1", borderBottom: "4px solid #44322d"} : {width: "250px", height: "20px", borderBottom: "4px solid #44322d"}}],
                            ["bar", "simplifier3"],
                            ["ex-buyable", 403],
                            ["style-row", [], {width: "250px", height: "20px", borderTop: "3px solid #44322d"}],
                        ], {width: "250px", background: "#88655B", border: "4px solid #44322d", borderRadius: "20px", margin: "5px"}],
                        ["top-column", [
                            ["style-column", [
                                ["raw-html", () => {return getBuyableAmount("tad", 404).gt(0) ? "<h3>4th Simplifier</h3><hr style='border:1px solid #44322d'>Reduces 3rd Simplifier cooldown by /" + formatSimple(player.tad.simplifiers[3].effect) : ""}, {color: "rgba(0,0,0,0.8)", fontSize: "14px", fontFamily: "monospace"}],
                            ], () => {return getBuyableAmount("tad", 404).gt(0) ? {width: "250px", height: "60px", lineHeight: "1", borderBottom: "4px solid #44322d"} : {width: "250px", height: "20px", borderBottom: "4px solid #44322d"}}],
                            ["bar", "simplifier4"],
                            ["ex-buyable", 404],
                            ["style-row", [], {width: "250px", height: "20px", borderTop: "3px solid #44322d"}],
                        ], {width: "250px", background: "#88655B", border: "4px solid #44322d", borderRadius: "20px", margin: "5px"}],
            ...
            "Alteration": {
                ...
                content: [
                    ...
                    ["blank", "10px"],
                    ["style-row", [
                        ["rounded-ex-buyable", 501], ["rounded-ex-buyable", 502], ["rounded-ex-buyable", 503],
                        ["rounded-ex-buyable", 504], ["rounded-ex-buyable", 505], ["rounded-ex-buyable", 506],
                    ], {maxWidth: "700px"}],
                ],
            },
            "Exponentiation": {
                ...
                content: [
                    ["row", [
                        ["raw-html", () => {return player.tad.magnification.neq(1) ? "You are at <h3>" + formatWhole(player.tad.exponentiate) + "</h3> exponentiations." : "You are at <h3>" + formatWhole(player.tad.exponentiate) + "</h3> exponentiation." }, {color: "black", fontSize: "24px", fontFamily: "monospace"}],
                        ["raw-html", () => {return "(+" + formatWhole(player.tad.exponentiateGain) + ")"}, () => {
                            let look = {color: "black", fontSize: "24px", fontFamily: "monospace", marginLeft: "10px"}
                            player.tad.exponentiateGain.gt(0) ? look.color = "black" : look.color = "#666"
                            return look
                        }],
                    ]],
                    ["raw-html", () => {return "Total exponentiations boost matter by ^" + formatSimple(player.tad.exponentiateEffect, 3)}, {color: "black", fontSize: "20px", fontFamily: "monospace"}],
                    ["blank", "10px"],
                    ["clickable", 41],
                    ["blank", "10px"],
                    ["clickable", 42],
            ...
            "Optimization": {
                buttonStyle() { return { color: "black", borderRadius: "5px" }},
                unlocked() { return true }, //a75553
                content: [
                    ["raw-html", () => {return "You have " + formatSimple(player.tad.infinitum, 1) + " infinitum"}, {color: "black", fontSize: "16px", fontFamily: "monospace"}],
                    ["raw-html", () => {return player.tad.optimization.neq(1) ? "You are at <h3>" + formatWhole(player.tad.optimization) + "</h3> optimizations." : "You are at <h3>" + formatWhole(player.tad.optimization) + "</h3> optimization." }, {color: "black", fontSize: "24px", fontFamily: "monospace"}],
                    ["blank", "10px"],
                    ["clickable", 51],
                    ["blank", "25px"],
                    ["top-column", [
                        ["row", [
                            ["style-column", [
                                ["style-column", [
                                    ["raw-html", "Accumulator Autobuyer", {color: "rgba(0,0,0,0.8)", fontSize: "18px", fontFamily: "monospace"}],
                                ], {width: "250px", height: "40px", borderBottom: "4px solid #532a29"}],
                                ["bar", "autoAccumulator"],
                                ["clickable", 1001],
                                ["style-row", [], {width: "250px", height: "3px", background: "#532a29"}],
                                ["ex-buyable", 1001],
                            ], () => {return player.tad.optimization.gte(1) ? {width: "250px", height: "250px", background: "#a75553", border: "4px solid #532a29"} : {display: "none !important"}}],
                            ["top-column", [
                                ["style-column", [
                                    ["style-column", [
                                        ["raw-html", "Automatically<br>Domain Collapse", {color: "rgba(0,0,0,0.8)", fontSize: "18px", fontFamily: "monospace"}],
                                    ], {width: "250px", height: "46px", borderBottom: "4px solid #532a29"}],
                                    ["clickable", 1002],
                                ], () => {return player.tad.optimization.gte(2) ? {width: "250px", height: "87px", background: "#a75553", border: "4px solid #532a29", borderRadius: "20px", marginLeft: "10px"} : {display: "none !important"}}],
                                ["blank", "6px"],
                                ["style-column", [
                                    ["raw-html", "Accumulators no longer spend matter", {color: "rgba(0,0,0,0.8)", fontSize: "18px", fontFamily: "monospace"}],
                                ], () => {return player.tad.optimization.gte(3) ? {width: "250px", height: "48px", background: "#a75553", border: "4px solid #532a29", borderRadius: "20px", marginLeft: "10px"} : {display: "none !important"}}],
                                ["blank", "6px"],
                                ["style-column", [
                                    ["style-column", [
                                        ["raw-html", "Automatically<br>Gain Compressions", {color: "rgba(0,0,0,0.8)", fontSize: "18px", fontFamily: "monospace"}],
                                    ], {width: "250px", height: "46px", borderBottom: "4px solid #532a29"}],
                                    ["clickable", 1003],
                        ...
                        ["style-row", [
                            ...
                            ["top-column", [
                                ["style-column", [
                                    ["raw-html", "Amplifiers no longer spend matter", {color: "rgba(0,0,0,0.8)", fontSize: "18px", fontFamily: "monospace"}],
                                ], () => {return player.tad.optimization.gte(7) ? {width: "250px", height: "50px", background: "#a75553", border: "4px solid #532a29", borderRadius: "20px", marginLeft: "10px"} : {display: "none !important"}}],
                                ["blank", "6px"],
                                ["top-column", [
                                    ["style-column", [
                                        ["raw-html", "Automatically<br>Buy Compressors", {color: "rgba(0,0,0,0.8)", fontSize: "18px", fontFamily: "monospace"}],
                                    ], {width: "250px", height: "46px", borderBottom: "4px solid #532a29"}],
                                    ["clickable", 1010],
                                    ["style-row", [
                                        ["clickable", 1011], ["clickable", 1012], ["clickable", 1013],
                                        ["clickable", 1014], ["clickable", 1015], ["clickable", 1016],
                                    ], {width: "250px", height: "80px", borderTop: "4px solid #532a29", borderBottom: "4px solid #532a29"}]
                                ], () => {return player.tad.optimization.gte(8) ? {width: "250px", height: "186px", background: "#a75553", border: "4px solid #532a29", borderRadius: "20px", marginLeft: "10px"} : {display: "none !important"}}],
                            ], () => {return player.tad.optimization.gte(7) ? {height: "258px"} : {}}],
                            ["top-column", [
                                ["style-column", [
                                    ["style-column", [
                                        ["raw-html", "Automatically Magnify", {color: "rgba(0,0,0,0.8)", fontSize: "18px", fontFamily: "monospace"}],
                                    ], {width: "250px", height: "36px", borderBottom: "4px solid #532a29"}],
                                    ["clickable", 1005],
                                    ["style-row", [], {width: "250px", height: "4px", background: "#532a29"}],
                                    ["clickable", 1006],
                                ], () => {return player.tad.optimization.gte(9) ? {width: "250px", height: "118px", background: "#a75553", border: "4px solid #532a29", borderRadius: "20px", marginLeft: "10px"} : {display: "none !important"}}],
                                ["blank", "6px"],
                                ["style-column", [
                                    ["style-column", [
                                        ["raw-html", "Automatically<br>Expand Domain", {color: "rgba(0,0,0,0.8)", fontSize: "18px", fontFamily: "monospace"}],
                                    ], {width: "250px", height: "36px", lineHeight: "1", borderBottom: "4px solid #532a29"}],
                                    ["clickable", 1020],
                                    ["style-row", [
                                        ["clickable", 1021], ["clickable", 1022], ["clickable", 1023],
                                    ], {width: "250px", height: "37px", borderTop: "4px solid #532a29"}],
                                ], () => {return player.tad.optimization.gte(10) ? {width: "250px", height: "118px", background: "#a75553", border: "4px solid #532a29", borderRadius: "20px", marginLeft: "10px"} : {display: "none !important"}}],
                            ], () => {return player.tad.optimization.gte(9) ? {height: "258px"} : {}}],
                        ], () => {return player.tad.optimization.gte(6) ? {marginTop: "10px"} : {}}],
                        ["style-row", [
                            ["style-column", [
                                ["style-column", [
                                    ["raw-html", "T2 Infinity Automation", {color: "rgba(0,0,0,0.8)", fontSize: "18px", fontFamily: "monospace"}],
                                ], {width: "250px", height: "33px", borderBottom: "4px solid #532a29"}],
                                ["ex-buyable", 1005],
                                ["style-row", [], {width: "250px", height: "3px", background: "#532a29"}],
                                ["ex-buyable", 1006],
