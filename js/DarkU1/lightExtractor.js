addLayer("le", {
    ...
    update(delta) {
        ...
        if (hasMilestone("prj", 107)) uniDiv = uniDiv.mul(player.prj.milestone107Effect)
        ...
        if (player.sme.autoLeaveToggle && player.le.starmetalAlloyToGetTrue.gte(player.sme.leaveAmount) && !player.pet.legPetTimers[0].active) {
            if (player.tempPaused) pauseUniverseAll(["D1", "DA", "U3", "A2", "DS"], "unpause", true)
    ...
    clickables: {
        11: {
            ...
            canClick() { return !player.pet.legPetTimers[0].active && player.du.points.gte(player.le.starmetalAlloyReq) && !player.le.universeResetSafety },
        ...
        13: {
            ...
            canClick() { return player.pet.legPetTimers[0].active && player.du.points.gte(player.le.eclipseShardsReq) && !player.le.universeResetSafety },
            unlocked() { return true },
            onClick() {
                player.le.resetAmount = player.le.resetAmount.add(1)
                if (player.le.highestReset.lt(player.le.resetAmount)) player.le.highestReset = player.le.resetAmount
