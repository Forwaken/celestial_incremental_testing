addLayer("pu", {
    ...
    clickables: {
        ...
        101: {
            title() {return !player.pu.clickables[101] ? "Highlight tab button when you can select<br>[ENABLED]" : "Highlight tab button when you can select<br>[DISABLED]"},
            canClick: true,
            unlocked: true,
            onClick() {
                if (player.pu.clickables[101]) {
                    player.pu.clickables[101] = false
                } else {
                    player.pu.clickables[101] = true
                }
            },
            style: {width: "400px", minHeight: "50px", color: "white", border: "2px solid #384166", borderRadius: "10px"},
        },
    },
    levelables: {
        0: {
        ...
        101: {
            ...
            // LEVEL CODE
            xpReq() {
                if (getLevelableAmount(this.layer, this.id).lt(4)) return getLevelableAmount(this.layer, this.id).add(1).mul(2).floor()
                if (getLevelableAmount(this.layer, this.id).gte(4)) return Decimal.pow(2, getLevelableAmount(this.layer, this.id)).div(1.4).floor()
            },
            currency() { return getLevelableXP(this.layer, this.id) },
            // STYLE CODE
            barStyle() { return {backgroundColor: "#1a3b0f"}},
        ...
        102: {
            ...
            // CLICK CODE
            unlocked() {return player.le.highestReset.gte(2) || this.canClick()},
            canSelect() {return player.le.resetAmount.gte(2)},
            canClick() {return getLevelableXP(this.layer, this.id).gt(0) || getLevelableAmount(this.layer, this.id).gt(0) || getLevelableTier(this.layer, this.id, true)},
            onClick() {return layers[this.layer].levelables.index = this.id},
            // LEVEL CODE
            xpReq() {
        ...
        106: {
            ...
            // CLICK CODE
            unlocked() {return (hasUpgrade("sma", 12) && player.le.highestReset.gte(3)) || this.canClick()},
            canSelect() {return hasUpgrade("sma", 12) && player.le.resetAmount.gte(3) && (!player.pet.legPetTimers[0].active || player.dotf.penumbral)},
            challengeType: 'SM',
        ...
        107: {
            ...
            description() {
                let str = [
                    ...
                    !getLevelableTier(this.layer, this.id, true) ? "</span>" : "",
                    "<u>Passive</u><br>",
                    "x" + format(this.effect()[1]) + " to pre-power " + player.h.stageName[1] + " resources",
                    getLevelableAmount(this.layer, this.id).gte(10) ? "<br><div style='font-size:10px;color:red'>[EFFECTS SOFTCAPPED]</div>" : "",
                ]
                return str.join("")
            },
        ...
        108: {
            ...
            // CLICK CODE
            unlocked() {return (hasUpgrade("sma", 16) && player.le.highestReset.gte(4)) || this.canClick()},
            canSelect() {return hasUpgrade("sma", 16) && player.le.resetAmount.gte(4)},
            canClick() {return getLevelableXP(this.layer, this.id).gt(0) || getLevelableAmount(this.layer, this.id).gt(0) || getLevelableTier(this.layer, this.id, true)},
            onClick() {return layers[this.layer].levelables.index = this.id},
            // LEVEL CODE
            xpReq() {
        ...
        200: {
            ...
            effect() {
                let eff = [new Decimal(1), new Decimal(1), new Decimal(1)]
                let time1 = player.le.timeSinceReset.gt(300) ? player.le.timeSinceReset.log(300).pow(0.5).mul(300) : player.le.timeSinceReset
                eff[0] = Decimal.pow(1.15, time1).pow(this.effectScale()).pow(player.pu.rareRaise)
                let time2 = player.le.timeSinceEnter.gt(1800) ? player.le.timeSinceEnter.log(1800).pow(0.5).mul(1800) : player.le.timeSinceEnter
        ...
        203: {
            ...
            description() {
                let str = [
                    ...
                    !getLevelableTier(this.layer, this.id, true) ? "</span>" : "",
                    "<u>Passive</u><br>",
                    "x" + format(this.effect()[2]) + " to " + player.h.stageName[1] + " power",
                    getLevelableAmount(this.layer, this.id).gte(10) ? "<br><div style='font-size:10px;color:red'>[EFFECTS SOFTCAPPED]</div>" : "",
                ]
                return str.join("")
            },
            ...
            canSelect() {return hasUpgrade("sma", 14) && player.le.resetAmount.gte(3) && (!player.pet.legPetTimers[0].active || player.dotf.penumbral)},
            challengeType: 'SM',
            canClick() {return getLevelableXP(this.layer, this.id).gt(0) || getLevelableAmount(this.layer, this.id).gt(0) || getLevelableTier(this.layer, this.id, true)},
            onClick() {return layers[this.layer].levelables.index = this.id},
            // LEVEL CODE
        ...
        205: {
            ...
            // CLICK CODE
            unlocked() {return hasUpgrade("sma", 16) && player.le.highestReset.gte(4) || this.canClick()},
            canSelect() {return hasUpgrade("sma", 16) && player.le.resetAmount.gte(4) && (!player.pet.legPetTimers[0].active || player.dotf.penumbral)},
            challengeType: 'SM',
            canClick() {return getLevelableXP(this.layer, this.id).gt(0) || getLevelableAmount(this.layer, this.id).gt(0) || getLevelableTier(this.layer, this.id, true)},
            onClick() {return layers[this.layer].levelables.index = this.id},
            // LEVEL CODE
        ...
        209: {
            ...
            description() {
                let str = [
                    ...
                    !getLevelableTier(this.layer, this.id, true) ? "</span>" : "",
                    "<u>Passive</u><br>",
                    "x" + format(this.effect()[1]) + " to " + player.h.stageName[1] + " points",
                    getLevelableAmount(this.layer, this.id).gte(10) ? "<br><div style='font-size:10px;color:red'>[EFFECTS SOFTCAPPED]</div>" : "",
                ]
                return str.join("")
            },
        ...
        210: {
            ...
            description() {
                let str = [
                    ...
                    !getLevelableTier(this.layer, this.id, true) ? "</span>" : "",
                    "<u>Passive</u><br>",
                    "^" + format(this.effect()[1], 3) + " to " + player.h.stageName[1] + " power",
                    getLevelableAmount(this.layer, this.id).gte(10) ? "<br><div style='font-size:10px;color:red'>[EFFECTS SOFTCAPPED]</div>" : "",
                ]
                return str.join("")
            },
        ...
        214: {
            ...
            effect() {
                let eff = [new Decimal(1), new Decimal(1)]
                eff[0] = player.dt.timeEnergy.pow(0.15).div(2).add(1).pow(this.effectScale()).pow(player.pu.rareRaise)
        ...
        215: {
            ...
            effect() {
                let eff = [new Decimal(1), new Decimal(1)]
                eff[0] = player.db.boosters.div(30).pow(2).add(1).pow(this.effectScale()).pow(player.pu.rareRaise)
        ...
        216: {
            image() {return this.canClick() ? "resources/Punchcards/rarePunchcard16.png" : "resources/Punchcards/lockedPunchcard.png"},
            title() {
                let str = "Boosters based on Generators"
                if (getLevelableTier(this.layer, this.id, true)) {str = str.concat("<small> [ACTIVE]</small>")} else {str = str.concat("<small style='color:gray'> [INACTIVE]</small>")}
                return str
            },
            description() {
                let str = [
                    !getLevelableTier(this.layer, this.id, true) ? "<span style='color:gray'>" : "",
                    "<u>Active</u><br>",
                    "/" + format(this.effect()[0]) + " to booster requirement. <small>(Based on generators)</small><br>",
                    !getLevelableTier(this.layer, this.id, true) ? "</span>" : "",
                    "<u>Passive</u><br>",
                    "/" + format(this.effect()[1]) + " to vex requirement",
                    getLevelableAmount(this.layer, this.id).gte(10) ? "<br><div style='font-size:10px;color:red'>[EFFECTS SOFTCAPPED]</div>" : "",
                ]
                return str.join("")
            },
            ...
            // CLICK CODE
            unlocked() {return (player.dotf.penumbral && !player.pet.legPetTimers[0].active && player.le.resetAmount.gte(3)) || this.canClick()},
            canSelect() {return player.dotf.penumbral && !player.pet.legPetTimers[0].active && player.le.resetAmount.gte(3)},
            challengeType: 'SM',
            canClick() {return getLevelableXP(this.layer, this.id).gt(0) || getLevelableAmount(this.layer, this.id).gt(0) || getLevelableTier(this.layer, this.id, true)},
            onClick() {return layers[this.layer].levelables.index = this.id},
            // LEVEL CODE
            xpReq() {
                return Decimal.pow(3, getLevelableAmount(this.layer, this.id).add(30)).floor()
            },
            currency() { return getLevelableXP(this.layer, this.id) },
            // STYLE CODE
            barStyle() { return {backgroundColor: "#1a3b0f"}},
        ...
        217: {
            image() {return this.canClick() ? "resources/Punchcards/rarePunchcard17.png" : "resources/Punchcards/lockedPunchcard.png"},
            title() {
                let str = "Generators based on Boosters"
                if (getLevelableTier(this.layer, this.id, true)) {str = str.concat("<small> [ACTIVE]</small>")} else {str = str.concat("<small style='color:gray'> [INACTIVE]</small>")}
                return str
            },
            description() {
                let str = [
                    !getLevelableTier(this.layer, this.id, true) ? "<span style='color:gray'>" : "",
                    "<u>Active</u><br>",
                    "x" + format(this.effect()[0]) + " to generators. <small>(Based on boosters)</small><br>",
                    !getLevelableTier(this.layer, this.id, true) ? "</span>" : "",
                    "<u>Passive</u><br>",
                    "^" + format(this.effect()[1]) + " to boons",
                    getLevelableAmount(this.layer, this.id).gte(10) ? "<br><div style='font-size:10px;color:red'>[EFFECTS SOFTCAPPED]</div>" : "",
                ]
                return str.join("")
            },
            ...
            effect() {
                let eff = [new Decimal(1), new Decimal(1)]
                eff[0] = Decimal.pow(2, player.db.boosters).pow(this.effectScale()).pow(player.pu.rareRaise)
                if (getLevelableAmount(this.layer, this.id).lt(10)) eff[1] = getLevelableAmount(this.layer, this.id).div(100).add(1)
                else eff[1] = getLevelableAmount(this.layer, this.id).div(500).add(1.08)
                return eff
            },
            // CLICK CODE
            unlocked() {return (player.dotf.penumbral && player.pet.legPetTimers[0].active && player.le.resetAmount.gte(3)) || this.canClick()},
            ...
            // LEVEL CODE
            xpReq() {
                return Decimal.pow(2.5, getLevelableAmount(this.layer, this.id).add(20)).floor()
            },
            currency() { return getLevelableXP(this.layer, this.id) },
            // STYLE CODE
            barStyle() { return {backgroundColor: "#1a3b0f"}},
                ...
                content: [
                    ["style-column", [
                        ...
                        ["always-scroll-column", [
                            ["style-column", [
                                ["raw-html", () => {return hasUpgrade("sma", 17) ? "Common (65%)" : hasUpgrade("sma", 14) ? "Common (75%)" : "Common"}, {color: "black", fontSize: "20px", fontFamily: "monospace"}],
                            ...
                            ["style-row", [
                                ...
                                ["levelable", 112], ["levelable", 113], ["levelable", 114],
                            ], () => {return hasUpgrade("sma", 14) ? {width: "600px", background: "repeating-linear-gradient(-45deg, #666666 0 15px, #808080 0 30px)", padding: "2px"} : {width: "600px", background: "repeating-linear-gradient(-45deg, #666666 0 15px, #808080 0 30px)", padding: "2px", borderBottom: "3px solid #7f7f7f"}}],
                            ...
                            ["style-row", [
                                ...
                                ["levelable", 212], ["levelable", 213], ["levelable", 214], ["levelable", 215],
                                ["levelable", 216], ["levelable", 217],
                            ], () => {return hasUpgrade("sma", 17) ? {width: "600px", background: "repeating-linear-gradient(-45deg, #705b1c 0 15px, #8c7223 0 30px)", padding: "2px"} : hasUpgrade("sma", 14) ? {width: "600px", background: "repeating-linear-gradient(-45deg, #666666 0 15px, #808080 0 30px)", padding: "2px", borderBottom: "3px solid #7f5f00"} : {display: "none !important"}}],
