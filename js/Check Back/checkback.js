...
addLayer("cb", {
    ...
    update(delta) {
        ...
        if (player.in.unlockedBreak) player.cb.baseESC = player.cb.baseESC.add(0.1)
        ...
        for (let i in player.cb.boostTimers) {
            ...
            if (player.ca.defeatedCante) player.cb.boostTimers[i].base = player.cb.boostTimers[i].base.mul(1.2)
    ...
    buyables: {
        11: {
            ...
            display() {
                if (tmp[this.layer].buyables[this.id].cost.gte(player.cb.totalxp)) {
                    return "which are multiplying " + player.h.stageName[1] + " points, rocket fuel, and dice points by x" + format(tmp[this.layer].buyables[this.id].effect) + ".\n\
        ...
        19: {
            ...
            display() {
                if (tmp[this.layer].buyables[this.id].cost.gte(player.cb.totalxp)) {
                    return "which are reducing pet shop scaling by /" + format(tmp[this.layer].buyables[this.id].effect) + ".\n\
                        Cost: " + formatWhole(layers.cb.xpToLevel(tmp[this.layer].buyables[this.id].cost)) + " Check Back Levels."
                } else {
                    let postCost = player.cb.totalxp.sub(tmp[this.layer].buyables[this.id].cost)
                    return "which are reducing pet shop scaling by /" + format(tmp[this.layer].buyables[this.id].effect) + ".\n\
    ...
    tabFormat: [
        ...
        ["raw-html", () => { return player.cb.highestLevel.lt(65) && player.cb.highestLevel.gte(50) ?  "You will unlock something at level 65! <small>[XP TAB]</small>" : "" }, {color: "white", fontSize: "24px", fontFamily: "monospace"}],
